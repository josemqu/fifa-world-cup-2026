                    ...
                    <tr key={u._id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            {u.nickname && (
                              <span className="inline-block mt-0.5 px-1.5 py-0.5 text-[10px] font-medium bg-slate-800 text-slate-400 rounded">
                                @{u.nickname}
                              </span>
                            )}
                          </div>
                        ...
                        <div className="space-y-1">
                          ...
                          {u.favoriteTeam && (
                            <div className="flex items-center gap-1.5 text-xs text-slate-400">
                              <Trophy className="w-3.5 h-3.5 text-slate-600 shrink-0" />
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center">
                          {completed ? (
                            <div className="flex items-center gap-1 text-xs text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-full">
                              <CheckCircle className="w-3.5 h-3.5" />
                              <span>Completo</span>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 text-xs text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-full">
                      ...
                      <td className="px-6 py-4 text-center font-semibold text-white">
                        {u.predictionCount > 0 ? (
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-indigo-500/10 text-indigo-400 text-xs">
                            {u.predictionCount}
                          </span>
                        ) : (
        ...
        {pagination && pagination.pages > 1 && (
          <div className="px-6 py-4 border-t border-slate-800 flex items-center justify-between">
            <p className="text-xs text-slate-500">
              Mostrando <span className="text-slate-350">{(page - 1) * pagination.limit + 1}</span> a{" "}
              <span className="text-slate-350">
                {Math.min(page * pagination.limit, pagination.total)}
              </span>{" "}
              de <span className="text-slate-350">{pagination.total}</span> usuarios
            </p>
            <div className="flex gap-2">
              <button
                disabled={page === 1}
                onClick={() => setPage((p) => p - 1)}
                className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
